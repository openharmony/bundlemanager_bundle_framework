    ...
    const std::string &hapFilePath, bool needFakeDecompression, bool useNewCodeDir, bool useModuleTmp)
{
    ...
    MessageParcel reply;
    MessageOption option;
    return TransactInstalldCmd(InstalldInterfaceCode::EXTRACT_RESOURCE_FILES, data, reply, option);
}
    ...
    const std::string &hapFilePath, int32_t userId)
{
    ...
    MessageParcel reply;
    MessageOption option;
    return TransactInstalldCmd(InstalldInterfaceCode::EXTRACT_ARK_PROFILE, data, reply, option);
}
...
ErrCode InstalldProxy::ExtractDriverKoFiles(const ExtractParam &extractParam,
    const std::unordered_multimap<std::string, std::string> &dirMap)
{
    MessageParcel data;
    INSTALLD_PARCEL_WRITE_INTERFACE_TOKEN(data, (GetDescriptor()));
    ...
    for (auto &[originalDir, destinedDir] : dirMap) {
        INSTALLD_PARCEL_WRITE(data, String16, Str8ToStr16(originalDir));
        INSTALLD_PARCEL_WRITE(data, String16, Str8ToStr16(destinedDir));
    }
    MessageParcel reply;
    MessageOption option(MessageOption::TF_SYNC);
    auto ret = TransactInstalldCmd(InstalldInterfaceCode::EXTRACT_TARGET_RAW_FILES, data, reply, option);
    if (ret != ERR_OK) {
        LOG_E(BMS_TAG_INSTALLD, "TransactInstalldCmd failed");
        return ret;
    }
