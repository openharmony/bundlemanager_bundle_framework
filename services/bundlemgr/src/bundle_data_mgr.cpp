...
#ifdef BMS_ENABLE_CLONE_FOR_ACCOUNT
        bool ret = ExplicitQueryAbilityInfoForClone(want, flags, requestUserId, abilityInfo, appIndex);
#else
...
#ifdef BMS_ENABLE_CLONE_FOR_ACCOUNT
    bool ret = ImplicitQueryAbilityInfosForClone(want, flags, requestUserId, abilityInfos, appIndex);
#else
...
#ifdef BMS_ENABLE_CLONE_FOR_ACCOUNT
bool BundleDataMgr::ExplicitQueryAbilityInfoForClone(const Want &want, int32_t flags, int32_t userId,
    AbilityInfo &abilityInfo, int32_t appIndex) const
{
    HITRACE_METER_NAME_EX(HITRACE_LEVEL_INFO, HITRACE_TAG_APP, __PRETTY_FUNCTION__, nullptr);
    ElementName element = want.GetElement();
    ...
    std::string moduleName = element.GetModuleName();
    LOG_D(BMS_TAG_QUERY,
        "ExplicitQueryAbilityInfoForClone bundleName:%{public}s moduleName:%{public}s abilityName:%{public}s",
        bundleName.c_str(), moduleName.c_str(), abilityName.c_str());
    LOG_D(BMS_TAG_QUERY, "flags:%{public}d userId:%{public}d", flags, userId);
    ...
    const InnerBundleInfo* innerBundleInfo = nullptr;
    if ((appIndex == 0) &&
        (ERR_OK != GetInnerBundleInfoForClone(bundleName, flags, innerBundleInfo, requestUserId))) {
        LOG_D(BMS_TAG_QUERY, "ExplicitQueryAbilityInfoForClone failed, bundleName:%{public}s", bundleName.c_str());
        return false;
    }
    InnerBundleInfo sandboxInfo;
    // explict query from sandbox manager
    ...
    if (!innerBundleInfo) {
        LOG_E(BMS_TAG_QUERY, "The InnerBundleInfo obtained by ExplicitQueryAbilityInfoForClone is null.");
        return false;
    }
    ...
    if (appIndex == Constants::MAIN_APP_INDEX &&
        ...
        if (enableAppIndex == Constants::ALL_CLONE_APP_INDEX) {
            LOG_NOFUNC_W(BMS_TAG_QUERY, "clone enable -> all disabled, bundleName:%{public}s", bundleName.c_str());
            return false;
        }
        queryAppIndex = enableAppIndex;
        LOG_NOFUNC_D(BMS_TAG_QUERY, "clone enable -> use appIndex:%{public}d, bundleName:%{public}s",
...
#ifdef BMS_ENABLE_CLONE_FOR_ACCOUNT
bool BundleDataMgr::ImplicitQueryAbilityInfosForClone(
    const Want &want, int32_t flags, int32_t userId, std::vector<AbilityInfo> &abilityInfos, int32_t appIndex) const
{
    HITRACE_METER_NAME_EX(HITRACE_LEVEL_INFO, HITRACE_TAG_APP, __PRETTY_FUNCTION__, nullptr);
    int32_t requestUserId = GetUserId(userId);
    ...
    if (!bundleName.empty()) {
        // query in current bundleName
        if (!ImplicitQueryCurAbilityInfosForClone(want, flags, requestUserId, abilityInfos, appIndex)) {
            LOG_D(BMS_TAG_QUERY, "ImplicitQueryCurAbilityInfosForClone failed bundleName:%{public}s",
                bundleName.c_str());
            return false;
        }
    } else {
...
#ifdef BMS_ENABLE_CLONE_FOR_ACCOUNT
bool BundleDataMgr::ImplicitQueryCurAbilityInfosForClone(const Want &want, int32_t flags, int32_t userId,
    std::vector<AbilityInfo> &abilityInfos, int32_t appIndex) const
{
    LOG_D(BMS_TAG_QUERY, "begin to ImplicitQueryCurAbilityInfosForClone");
    int32_t appFlags = (static_cast<uint32_t>(flags) & GET_ABILITY_INFO_WITH_DISABLE)
        ? GET_APPLICATION_INFO_WITH_DISABLE : GET_BASIC_APPLICATION_INFO;
    std::string bundleName = want.GetElement().GetBundleName();
    const InnerBundleInfo* innerBundleInfo = nullptr;
    if ((appIndex == 0) &&
        (ERR_OK != GetInnerBundleInfoForClone(bundleName, appFlags, innerBundleInfo, userId))) {
        LOG_W(BMS_TAG_QUERY, "ImplicitQueryCurAbilityInfosForClone failed bundleName:%{public}s", bundleName.c_str());
        return false;
    }
    InnerBundleInfo sandboxInfo;
    if (appIndex > 0) {
    ...
    if (!innerBundleInfo) {
        LOG_E(BMS_TAG_QUERY, "The InnerBundleInfo obtained by ImplicitQueryCurAbilityInfosForClone is null.");
        return false;
    }
    int32_t responseUserId = innerBundleInfo->GetResponseUserId(userId);
    std::vector<std::string> mimeTypes;
    ...
    const std::string &abilityUri, int32_t userId, AbilityInfo &abilityInfo) const
{
    ...
    for (const auto &item : bundleInfos_) {
        ...
        int32_t enableAppIndex = CloneForAccountUtil::GetEnabledCloneAppIndex(info, responseUserId);
        if (enableAppIndex == Constants::ALL_CLONE_APP_INDEX) {
            LOG_NOFUNC_W(BMS_TAG_QUERY, "car mode: maybe all apps are disabled, bundleName:%{public}s",
                info.GetBundleName().c_str());
            continue;
        }
        queryAppIndex = enableAppIndex;
        LOG_NOFUNC_D(BMS_TAG_QUERY, "car mode: use appIndex:%{public}d, bundleName:%{public}s",
            queryAppIndex, info.GetBundleName().c_str());
#else
    ...
    const std::string &appName, int32_t flags, const int userId, ApplicationInfo &appInfo) const
{
    ...
    const InnerBundleInfo* innerBundleInfo = nullptr;
#ifdef BMS_ENABLE_CLONE_FOR_ACCOUNT
    if (ERR_OK != GetInnerBundleInfoForClone(appName, flags, innerBundleInfo, requestUserId)) {
    ...
    if (!(static_cast<uint32_t>(flags) & GET_APPLICATION_INFO_WITH_DISABLE)) {
        int32_t enableAppIndex = CloneForAccountUtil::GetEnabledCloneAppIndex(*innerBundleInfo, responseUserId);
        if (enableAppIndex == Constants::ALL_CLONE_APP_INDEX) {
            LOG_NOFUNC_W(BMS_TAG_QUERY, "clone enable -> all disabled, bundleName:%{public}s", appName.c_str());
            return false;
        }
        queryAppIndex = enableAppIndex;
        LOG_NOFUNC_D(BMS_TAG_QUERY, "clone enable -> use appIndex:%{public}d, bundleName:%{public}s",
...
#ifdef BMS_ENABLE_CLONE_FOR_ACCOUNT
    auto ret = GetInnerBundleInfoForClone(appName, flag, innerBundleInfo, requestUserId, appIndex);
#else
    ...
    const std::string &bundleName, int32_t flags, BundleInfo &bundleInfo, int32_t userId) const
{
    ...
    const InnerBundleInfo *innerBundleInfo = nullptr;
#ifdef BMS_ENABLE_CLONE_FOR_ACCOUNT
    if (ERR_OK != GetInnerBundleInfoForClone(bundleName, flags, innerBundleInfo, requestUserId)) {
    ...
    const std::string &bundleName, int32_t flags, BundleInfo &bundleInfo, int32_t userId, int32_t appIndex) const
{
    ...
    const InnerBundleInfo* innerBundleInfo = nullptr;
#ifdef BMS_ENABLE_CLONE_FOR_ACCOUNT
    auto ret = GetInnerBundleInfoForClone(bundleName, flags, innerBundleInfo, requestUserId);
#else
    ...
    if (!(static_cast<uint32_t>(flags) & static_cast<uint32_t>(GetBundleInfoFlag::GET_BUNDLE_INFO_WITH_DISABLE))) {
        ...
        queryAppIndex = enableAppIndex;
        LOG_NOFUNC_D(BMS_TAG_QUERY, "clone enable -> enable appIndex:%{public}d, bundleName:%{public}s",
            queryAppIndex, bundleName.c_str());
    }
    ...
    const std::string &bundleName, int32_t flags, BundlePackInfo &bundlePackInfo, int32_t userId) const
{
    ...
    if (!(static_cast<uint32_t>(flags) & GET_APPLICATION_INFO_WITH_DISABLE)) {
        ...
        LOG_NOFUNC_D(BMS_TAG_QUERY, "car mode: use appIndex:%{public}d, bundleName:%{public}s",
            enableAppIndex, bundleName.c_str());
    }
#endif
    ...
    int32_t userId, int32_t flags) const
{
    ...
    for (const auto &infoItem : bundleInfos_) {
        ...
        int32_t responseUserId = innerBundleInfo.GetResponseUserId(requestUserId);
        ErrCode ret = CheckInnerBundleInfoWithFlags(innerBundleInfo, flags, responseUserId);
#ifdef BMS_ENABLE_CLONE_FOR_ACCOUNT
        if (ret == ERR_BUNDLE_MANAGER_APPLICATION_DISABLED) {
            if (CloneForAccountUtil::GetEnabledCloneAppIndex(innerBundleInfo,
                responseUserId) == Constants::ALL_CLONE_APP_INDEX) {
                continue;
            }
        } else if (ret != ERR_OK) {
            continue;
...
bool BundleDataMgr::GetDebugBundleList(std::vector<std::string> &bundleNames, int32_t userId) const
{
    ...
    for (const auto &infoItem : bundleInfos_) {
        const InnerBundleInfo &innerBundleInfo = infoItem.second;
        int32_t responseUserId = innerBundleInfo.GetResponseUserId(requestUserId);
        ErrCode ret = CheckInnerBundleInfoWithFlags(
            innerBundleInfo, BundleFlag::GET_BUNDLE_DEFAULT, responseUserId);
#ifdef BMS_ENABLE_CLONE_FOR_ACCOUNT
        if (ret == ERR_BUNDLE_MANAGER_APPLICATION_DISABLED) {
            if (CloneForAccountUtil::GetEnabledCloneAppIndex(innerBundleInfo,
                responseUserId) == Constants::ALL_CLONE_APP_INDEX) {
...
#ifdef BMS_ENABLE_CLONE_FOR_ACCOUNT
    if (innerBundleUserInfo.bundleUserInfo.enabled) {
        return true;
    }
    for (const auto &cloneItem : innerBundleUserInfo.cloneInfos) {
        if (cloneItem.second.enabled) {
            gids = cloneItem.second.gids;
            LOG_NOFUNC_D(BMS_TAG_QUERY, "GetBundleGids clone enable -> use gids, appIndex:%{public}d, "
                "bundleName:%{public}s", cloneItem.second.appIndex, bundleName.c_str());
            return true;
        }
    }
#endif
    ...
    const std::string &abilityName, std::string &label) const
{
    ...
    int32_t responseUserId = -1;
    {
...
#ifdef BMS_ENABLE_CLONE_FOR_ACCOUNT
        ErrCode ret =
            GetInnerBundleInfoForClone(bundleName, BundleFlag::GET_BUNDLE_DEFAULT, innerBundleInfo, requestUserId);
#else
        ErrCode ret =
        ...
        LOG_NOFUNC_D(BMS_TAG_QUERY, "car mode: use appIndex:%{public}d, bundleName:%{public}s",
            queryAppIndex, bundleName.c_str());
        ret = innerBundleInfo->IsAbilityEnabledV9(abilityInfo, responseUserId, isEnable, queryAppIndex);
#else
    ...
    const AbilityInfo &abilityInfo, HapModuleInfo &hapModuleInfo, int32_t userId) const
{
    ...
    queryAppIndex = enableAppIndex;
    LOG_NOFUNC_D(BMS_TAG_QUERY, "car mode: use appIndex:%{public}d, bundleName:%{public}s",
        queryAppIndex, abilityInfo.bundleName.c_str());
    auto module = innerBundleInfo.FindHapModuleInfo(abilityInfo.package, responseUserId, queryAppIndex);
#else
    ...
    const std::string &bundleName, Want &want, int32_t userId) const
{
    ...
    const InnerBundleInfo *innerBundleInfo = nullptr;
#ifdef BMS_ENABLE_CLONE_FOR_ACCOUNT
    ErrCode ret = GetInnerBundleInfoForClone(
    ...
    int32_t enableAppIndex = CloneForAccountUtil::GetEnabledCloneAppIndex(*innerBundleInfo, responseUserId);
    if (enableAppIndex == Constants::ALL_CLONE_APP_INDEX) {
        LOG_NOFUNC_W(BMS_TAG_QUERY, "GetLaunchWantForBundle clone enable -> enable all clone apps disabled, "
            "bundleName:%{public}s", bundleName.c_str());
        return ERR_BUNDLE_MANAGER_APPLICATION_DISABLED;
    }
    want.SetParam(Constants::APP_INDEX, enableAppIndex);
    LOG_NOFUNC_D(BMS_TAG_QUERY, "GetLaunchWantForBundle clone enable -> use appIndex:%{public}d, bundleName:%{public}s",
        enableAppIndex, bundleName.c_str());
#endif // BMS_ENABLE_CLONE_FOR_ACCOUNT
    ...
    const std::string &bundleName, Want &want, int32_t userId) const
{
    ...
    int32_t enableAppIndex = CloneForAccountUtil::GetEnabledCloneAppIndex(*innerBundleInfo, responseUserId);
    if (enableAppIndex == Constants::ALL_CLONE_APP_INDEX) {
        LOG_NOFUNC_W(BMS_TAG_QUERY, "GetLaunchWantForBundleSync clone enable -> enable all clone apps disabled, "
            "bundleName:%{public}s", bundleName.c_str());
        return ERR_BUNDLE_MANAGER_APPLICATION_DISABLED;
    }
    want.SetParam(Constants::APP_INDEX, enableAppIndex);
    LOG_NOFUNC_D(BMS_TAG_QUERY, "GetLaunchWantForBundleSync clone enable -> use appIndex:%{public}d, "
        "bundleName:%{public}s", enableAppIndex, bundleName.c_str());
#endif // BMS_ENABLE_CLONE_FOR_ACCOUNT
    ...
    const int32_t flags, InnerBundleInfo& info, int32_t userId) const
{
    ...
    const InnerBundleInfo* innerBundleInfo = nullptr;
#ifdef BMS_ENABLE_CLONE_FOR_ACCOUNT
    if (ERR_OK != GetInnerBundleInfoForClone(bundleName, flags, innerBundleInfo, userId)) {
        LOG_D(BMS_TAG_QUERY, "GetApplicationInfo failed, bundleName:%{public}s", bundleName.c_str());
        return false;
    }
    if (!(static_cast<uint32_t>(flags) &
          static_cast<uint32_t>(GetApplicationFlag::GET_APPLICATION_INFO_WITH_DISABLE))) {
        ...
        if (enableAppIndex == Constants::ALL_CLONE_APP_INDEX) {
            LOG_NOFUNC_W(
                BMS_TAG_QUERY, "car mode: maybe all apps are disabled, bundleName:%{public}s", bundleName.c_str());
            return false;
        }
        LOG_NOFUNC_D(BMS_TAG_QUERY, "car mode:bundleName:%{public}s", bundleName.c_str());
    }
#else
        ...
        const InnerBundleInfo *&info, int32_t userId, int32_t appIndex) const
{
    ...
    APP_LOGD("GetInnerBundleInfoForClone: %{public}s", bundleName.c_str());
    auto item = bundleInfos_.find(bundleName);
    if (item == bundleInfos_.end()) {
        APP_LOGW_NOFUNC("%{public}s not find", bundleName.c_str());
        return ERR_BUNDLE_MANAGER_BUNDLE_NOT_EXIST;
    ...
    const std::string &bundleName, int32_t userId, std::vector<ShortcutInfo> &shortcutInfos) const
{
    ...
    const InnerBundleInfo *innerBundleInfo = nullptr;
#ifdef BMS_ENABLE_CLONE_FOR_ACCOUNT
    if (ERR_OK != GetInnerBundleInfoForClone(
    ...
    int32_t storageAppIndex = Constants::MAIN_APP_INDEX;
#ifdef BMS_ENABLE_CLONE_FOR_ACCOUNT
    int32_t responseUserId = innerBundleInfo->GetResponseUserId(requestUserId);
    int32_t enableAppIndex = CloneForAccountUtil::GetEnabledCloneAppIndex(*innerBundleInfo, responseUserId);
    if (enableAppIndex == Constants::ALL_CLONE_APP_INDEX) {
    ...
    storageAppIndex = enableAppIndex;
    LOG_NOFUNC_D(BMS_TAG_QUERY, "car mode: use appIndex:%{public}d, bundleName:%{public}s",
        storageAppIndex, bundleName.c_str());
#endif
    ...
    if (innerBundleInfo->IsDualModeCloneApp()) {
        storageAppIndex = ServiceConstants::DUAL_MODE_CLONE_APP_INDEX;
    }
    ...
    const std::string &bundleName, int32_t userId, std::vector<ShortcutInfo> &shortcutInfos) const
{
    ...
    const InnerBundleInfo *innerBundleInfo = nullptr;
#ifdef BMS_ENABLE_CLONE_FOR_ACCOUNT
    ErrCode ret = GetInnerBundleInfoForClone(bundleName,
    ...
    int32_t enableAppIndex = CloneForAccountUtil::GetEnabledCloneAppIndex(*innerBundleInfo, responseUserId);
    if (enableAppIndex == Constants::ALL_CLONE_APP_INDEX) {
        LOG_NOFUNC_W(BMS_TAG_QUERY, "GetShortcutInfoV9 clone enable -> enable all clone apps disabled, "
            "bundleName:%{public}s", bundleName.c_str());
        return ERR_BUNDLE_MANAGER_APPLICATION_DISABLED;
    }
    for (auto &info : shortcutInfos) {
        info.appIndex = enableAppIndex;
    }
#endif
    ...
    ExtensionAbilityInfo &extensionAbilityInfo) const
{
    ...
    for (const auto &item : bundleInfos_) {
        ...
        info.GetApplicationInfo(
            ApplicationFlag::GET_APPLICATION_INFO_WITH_CERTIFICATE_FINGERPRINT, responseUserId,
            extensionAbilityInfo.applicationInfo, queryAppIndex);
#else
        info.GetApplicationInfo(
            ApplicationFlag::GET_APPLICATION_INFO_WITH_CERTIFICATE_FINGERPRINT, responseUserId,
            extensionAbilityInfo.applicationInfo);
...
#ifdef BMS_ENABLE_CLONE_FOR_ACCOUNT
        extensionAbilityInfo.appIndex = queryAppIndex;
#endif
        return true;
    }
    ...
    const std::string &bundleName, const std::string &moduleName, int32_t userId, const std::string &localeInfo) const
{
    BundleInfo bundleInfo;
    {
...
#ifdef BMS_ENABLE_CLONE_FOR_ACCOUNT
        if (ERR_OK !=
            GetInnerBundleInfoForClone(bundleName, BundleFlag::GET_BUNDLE_DEFAULT, innerBundleInfo, userId)) {
            APP_LOGW("can not find bundle %{public}s", bundleName.c_str());
            return nullptr;
        }
#else
        ...
        innerBundleInfo->GetBundleInfo(BundleFlag::GET_BUNDLE_DEFAULT, bundleInfo, responseUserId, enableAppIndex);
#else
...
void BundleDataMgr::GetBundleInfoList(const std::string &bundleName, std::vector<InnerBundleInfo>& bundleInfoList)
{
    ...
    auto tempItem = tempBundleInfos_.find(bundleName);
    if (tempItem != tempBundleInfos_.end()) {
        bundleInfoList.emplace_back(tempItem->second);
    }
}
...
ErrCode BundleDataMgr::GetAppProvisionInfoInDevice(const std::string &bundleName, int32_t userId,
    std::vector<AppProvisionInfo> &appProvisionInfos)
{
    if (!HasUserId(userId)) {
        APP_LOGW("GetAppProvisionInfoInDevice user is not existed. bundleName:%{public}s", bundleName.c_str());
        return ERR_BUNDLE_MANAGER_INVALID_USER_ID;
    }
    if (bundleName.empty()) {
    ...
    for (const auto& bundleInfo : bundleInfoList) {
        ...
        if (bundleType == BundleType::SHARED || bundleType == BundleType::SKILL ||
            responseUserId == Constants::INVALID_USERID) {
            APP_LOGW("userId: %{public}d is invalid, and will find next bundleInfo", userId);
            continue;
        }
        ...
        if (!DelayedSingleton<AppProvisionInfoManager>::GetInstance()->GetAppProvisionInfo(
            effectiveBundleName, appProvisionInfo)) {
            APP_LOGW("effectiveBundleName:%{public}s GetAppProvisionInfoInDevice failed",
                effectiveBundleName.c_str());
            continue;
        }
        appProvisionInfo.bundleName = bundleName;
        appProvisionInfo.appIndex = isDualModeCloneApp ? ServiceConstants::DUAL_MODE_CLONE_APP_INDEX : 0;
...
ErrCode BundleDataMgr::GetAllAppProvisionInfoInDevice(
    const int32_t userId, std::vector<AppProvisionInfo> &appProvisionInfos)
{
    ...
    for (const auto& [bundleName, isDualModeCloneApp] : bundleInfoList) {
        ...
        if (!DelayedSingleton<AppProvisionInfoManager>::GetInstance()->GetAppProvisionInfo(
            effectiveBundleName, appProvisionInfo)) {
            APP_LOGW("effectiveBundleName:%{public}s GetAllAppProvisionInfoInDevice failed",
                effectiveBundleName.c_str());
            continue;
        }
        appProvisionInfo.bundleName = bundleName;
    ...
    const std::string &moduleName, std::string &profile, int32_t userId) const
{
    ...
    std::string hapPath;
    {
...
#ifdef BMS_ENABLE_CLONE_FOR_ACCOUNT
        int32_t queryAppIndex = Constants::MAIN_APP_INDEX;
        int32_t enableAppIndex = CloneForAccountUtil::GetEnabledCloneAppIndex(bundleInfo, responseUserId);
        if (enableAppIndex == Constants::ALL_CLONE_APP_INDEX) {
            LOG_NOFUNC_W(BMS_TAG_QUERY, "car mode: maybe all apps are disabled, bundleName:%{public}s",
                bundleName.c_str());
            return ERR_BUNDLE_MANAGER_APPLICATION_DISABLED;
        ...
        LOG_NOFUNC_D(BMS_TAG_QUERY, "car mode: use appIndex:%{public}d, bundleName:%{public}s",
            queryAppIndex, bundleName.c_str());
        ErrCode res = bundleInfo.GetApplicationEnabledV9(responseUserId, isEnabled, queryAppIndex);
#else
...
int32_t BundleDataMgr::GetUidByBundleName(const std::string &bundleName, int32_t userId, int32_t appIndex) const
{
    ...
    if (appIndex == Constants::ALL_CLONE_APP_INDEX) {
        ...
        if (enableAppIndex == Constants::ALL_CLONE_APP_INDEX) {
            ...
            LOG_NOFUNC_W(BMS_TAG_QUERY, "clone enable -> all disabled, fall back to main, bundleName:%{public}s",
                bundleName.c_str());
        } else {
            queryAppIndex = enableAppIndex;
            LOG_NOFUNC_D(BMS_TAG_QUERY, "clone enable -> use appIndex:%{public}d, bundleName:%{public}s",
    ...
    std::vector<BundleInfo> &bundleInfos, int32_t userId)
{
    ...
    for (const auto &item : bundleInfos_) {
        ...
        int32_t responseUserId = innerBundleInfo.GetResponseUserId(requestUserId);
        auto flag = GET_BASIC_APPLICATION_INFO;
        ErrCode ret = CheckInnerBundleInfoWithFlags(innerBundleInfo, flag, responseUserId);
#ifdef BMS_ENABLE_CLONE_FOR_ACCOUNT
        int32_t queryAppIndex = Constants::MAIN_APP_INDEX;
        if (ret == ERR_BUNDLE_MANAGER_APPLICATION_DISABLED) {
            int32_t enableAppIndex = CloneForAccountUtil::GetEnabledCloneAppIndex(innerBundleInfo, responseUserId);
        ...
        if (innerBundleInfo.GetBundleInfoV9(static_cast<int32_t>(GetBundleInfoFlag::GET_BUNDLE_INFO_WITH_APPLICATION),
            bundleInfo, responseUserId, queryAppIndex) != ERR_OK) {
            continue;
        }
#else
        if (innerBundleInfo.GetBundleInfoV9(static_cast<int32_t>(GetBundleInfoFlag::GET_BUNDLE_INFO_WITH_APPLICATION),
    ...
    const std::string &continueBundleName, std::vector<std::string> &bundleNames, int32_t userId)
{
    ...
    for (const auto &[key, innerInfo] : bundleInfos_) {
        int32_t responseUserId = innerInfo.GetResponseUserId(requestUserId);
        ErrCode ret = CheckInnerBundleInfoWithFlags(
            innerInfo, BundleFlag::GET_BUNDLE_WITH_ABILITIES, responseUserId);
#ifdef BMS_ENABLE_CLONE_FOR_ACCOUNT
        if (ret == ERR_BUNDLE_MANAGER_APPLICATION_DISABLED) {
            int32_t enableAppIndex = CloneForAccountUtil::GetEnabledCloneAppIndex(innerInfo, responseUserId);
            if (enableAppIndex == Constants::ALL_CLONE_APP_INDEX) {
                continue;
            }
        } else if (ret != ERR_OK) {
