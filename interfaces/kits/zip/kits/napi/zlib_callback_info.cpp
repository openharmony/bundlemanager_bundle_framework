    ...
    int32_t ret = uv_queue_work(
        ...
        [](uv_work_t* work, int status) {
            ...
            if (asyncCallbackInfo->deliverErrcode) {
                ...
                CHKRV_SCOPE(asyncCallbackInfo->env, napi_create_int32(asyncCallbackInfo->env,
                    asyncCallbackInfo->callbackResult, &result[0]), scope);
            }
            if (asyncCallbackInfo->isCallBack) {
                napi_value callback = 0;
                napi_value placeHolder = nullptr;
                CHKRV_SCOPE(asyncCallbackInfo->env, napi_get_reference_value(asyncCallbackInfo->env,
                    asyncCallbackInfo->callback, &callback), scope);
                ...
                if (asyncCallbackInfo->callback != nullptr) {
                    CHKRV_SCOPE(asyncCallbackInfo->env, napi_delete_reference(asyncCallbackInfo->env,
                        asyncCallbackInfo->callback), scope);
                }
            } else {
                if (asyncCallbackInfo->callbackResult == ERR_OK) {
