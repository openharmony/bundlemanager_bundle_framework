...
void BundleUtil::MakeFsConfig(const std::string &bundleName, const std::string &configPath, const std::string labelValue,
...
std::string BundleUtil::GetAbilityKey(const std::string &bundleName, const std::string &moduleName, const std::string &abilityName)
{
    return "";
}
...
bool BundleUtil::PatchReadWhiteListXml(std::unordered_map<uint64_t, std::vector<std::string>> &logicalIdWhiteListMap)
{
    ...
    logicalIdWhiteListMap.emplace(2, bundleNames);
    return true;
}
