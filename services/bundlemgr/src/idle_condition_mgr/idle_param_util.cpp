...
bool IdleParamUtil::IsFileCategoryScanDisabled()
{
    std::string higherVersionPath = GetHigherVersionPath();
    if (higherVersionPath.empty()) {
        APP_LOGE_NOFUNC("AppFileCategory higherVersionPath is empty");
        return false;
    }
    std::string filePath = higherVersionPath + SWITCH_OFF_LIST;
    std::ifstream inputFile(filePath);
    if (!inputFile.is_open()) {
        APP_LOGE_NOFUNC("AppFileCategory Can not open file: %{public}s", filePath.c_str());
        return false;
    }
    std::string line;
    while (std::getline(inputFile, line)) {
        Trim(line);
        if (line == FILE_CATEGORY_SCAN_FEATURE_OFF) {
            APP_LOGI_NOFUNC("AppFileCategory file category scan off switch found");
            return true;
        }
    }
    APP_LOGI_NOFUNC("AppFileCategory file category scan off switch not found");
    return false;
}
