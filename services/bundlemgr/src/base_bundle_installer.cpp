    ...
    const std::vector<std::string> &bundlePaths, const InstallParam &installParam, const Constants::AppType appType)
{
    ...
    bool hasEnableState = installParam.GetBundleEnableState(bundleEnableState);
    if (hasEnableState && result == ERR_OK && dataMgr_ != nullptr) {
        ...
        if (ret != ERR_OK) {
            ...
            if (queryRet != ERR_OK) {
                LOG_W(BMS_TAG_INSTALLER, "IsApplicationEnabled fallback failed, ret=%{public}d", queryRet);
    ...
    InnerBundleInfo &oldInfo, const InstallParam &installParam, int32_t &uid)
{
    ...
    if (isAppExist_) {
        ...
        if (!hasInstalledInUser_) {
            ...
            if (!CheckInstallOnKeepData(GetEffectiveBundleName(), installParam.isOTA, newInfos)) {
            ...
            ScopeGuard userGuard([&] {
                if (!dataMgr_->GetUninstallBundleInfoWithUserAndAppIndex(GetEffectiveBundleName(), userId_,
            ...
            if (!RecoverHapToken(GetEffectiveBundleName(), userId_, accessTokenIdEx, oldInfo, false)
    ...
    ScopeGuard userGuard([&] {
        if ((!hasInstalledInUser_ || (!isAppExist_)) &&
            !dataMgr_->GetUninstallBundleInfoWithUserAndAppIndex(GetEffectiveBundleName(), userId_,
                Constants::INITIAL_APP_INDEX)) {
            InstallParam installParam;
            RemoveBundleUserData(oldInfo, installParam);
        }
    ...
    const InstallParam &installParam, const Constants::AppType appType, int32_t &uid, bool isRecover)
{
    ...
    ProcessOldCodePath(GetEffectiveBundleName(), isFeatureNeedUninstall_);
    ...
    InnerBundleInfo &oldInfo)
{
    ...
    if (!isAppExist_) {
        ...
        bool isKeepData = dataMgr_->GetUninstallBundleInfoWithUserAndAppIndex(GetEffectiveBundleName(), userId_,
            Constants::INITIAL_APP_INDEX);
    ...
    const std::string &bundleName, const InstallParam &installParam, int32_t &uid)
{
    ...
    if (isMultiUser) {
        ...
        BundleResourceHelper::DeleteBundleResourceInfo(oldInfo, userId_, true);
    ...
    const std::string &bundleName, const std::string &modulePackage, const InstallParam &installParam, int32_t &uid)
{
    ...
    ScopeGuard stateGuard([&] {
        dataMgr_->UpdateBundleInstallState(effectiveBundleName, InstallState::INSTALL_SUCCESS);
    });
    ...
    oldInfo.SetInstallMark(effectiveBundleName, modulePackage, InstallExceptionStatus::UNINSTALL_PACKAGE_START);
    if (!dataMgr_->SaveInnerBundleInfo(oldInfo)) {
        LOG_E(BMS_TAG_INSTALLER, "save install mark failed");
        return ERR_APPEXECFWK_UPDATE_BUNDLE_ERROR;
    }
    ...
    oldInfo.SetInstallMark(effectiveBundleName, modulePackage, InstallExceptionStatus::INSTALL_FINISH);
    ...
    const std::string &bundleName, const InstallParam &installParam, int32_t &uid)
{
...

    {
        ...
        // Preset primary variant (original name) in secondary mode: a generic lookup hits the
        // clone variant in bundleInfos_, but the primary is actually in tempBundleInfos_.
        // Re-fetch the primary (the clone key was already routed by InitTempBundleFromCache).
        if (isAppExist_ && DualModeHelper::IsSecondaryMode()
            && !DualModeHelper::IsDualModeCloneKey(bundleName)
            && oldInfo.IsDualModeCloneApp()) {
            InnerBundleInfo primaryInfo;
            if (dataMgr_->FetchTempBundleInfo(bundleName, primaryInfo)) {
                oldInfo = primaryInfo;
                tempInfo_.SetTempBundleInfo(oldInfo);
            } else {
                // Primary variant is absent (first-boot primary failed but clone succeeded):
                // treat as a fresh install so the !isAppExist_ branch re-installs the primary
                // instead of silently misrouting user-data creation to the clone.
                LOG_W(BMS_TAG_INSTALLER, "primary variant absent for %{public}s, fallback to fresh install",
                    bundleName.c_str());
                isAppExist_ = false;
            }
        }
        if (isAppExist_) {
            ...
            ScopeGuard userGuard([&] {
                if (!dataMgr_->GetUninstallBundleInfoWithUserAndAppIndex(effectiveBundleName, userId_,
                    Constants::INITIAL_APP_INDEX)) {
                    InstallParam installParam;
                    RemoveBundleUserData(oldInfo, installParam);
                }
            ...
            Security::AccessToken::HapInfoCheckResult checkResult;
            isKeepTokenId_ = oldInfo.HasKeepTokenIdMetadata();
            if (!RecoverHapToken(effectiveBundleName, userId_, accessTokenIdEx, oldInfo)) {
...
ErrCode BaseBundleInstaller::ProcessBundleInstallStatus(InnerBundleInfo &info, int32_t &uid, const bool isDebugGrant)
{
    ...
    Security::AccessToken::HapInfoCheckResult checkResult;
    isKeepTokenId_ = info.HasKeepTokenIdMetadata();
    if (!RecoverHapToken(GetEffectiveBundleName(), userId_, accessTokenIdEx, info, isDebugGrant)) {
    ...
    ScopeGuard bundleGuard([&] {
        RemoveBundleAndDataDir(info, dataMgr_->GetUninstallBundleInfoWithUserAndAppIndex(
            GetEffectiveBundleName(), userId_, Constants::INITIAL_APP_INDEX));
...
ErrCode BaseBundleInstaller::CreateBundleCodeDir(InnerBundleInfo &info) const
{
    ...
    ErrCode result =
        InstalldClient::GetInstance()->CreateBundleDir(effectiveBundleName, BundleDirScene::BUNDLE_CODE_DIR,
            appCodePath);
    if (result != ERR_OK) {
...
ErrCode BaseBundleInstaller::CreateBundleDataDir(InnerBundleInfo &info) const
{
    ...
    // dual-mode: compute the effective (prefixed) bundle name once and reuse it below for uid
    // generation, fs config, directory creation, quota, ark profile and database dir.
    std::string effectiveBundleName = GetEffectiveBundleName(info);
    // === DUAL_MODE: secondary-mode apps derive an independent uid from the prefixed name
    // (a distinct bundleId in bundleIdMap_). Restore the original bundleName afterwards so the
    // prefixed name is NOT persisted into userInfo (only the uid is). effectiveBundleName equals
    // the original name in primary mode, so the assignment is a no-op there.
    std::string originalBundleName = newInnerBundleUserInfo.bundleName;
    newInnerBundleUserInfo.bundleName = effectiveBundleName;
    ErrCode ret = dataMgr_->GenerateUidAndGid(newInnerBundleUserInfo);
    newInnerBundleUserInfo.bundleName = originalBundleName;
    if (ret != ERR_OK) {
        LOG_E(BMS_TAG_INSTALLER, "fail to generate uid and gid");
        return ret;
    }
    BundleUtil::MakeFsConfig(effectiveBundleName, ServiceConstants::HMDFS_CONFIG_PATH,
        info.GetAppProvisionType(), Constants::APP_PROVISION_TYPE_FILE_NAME);
    CreateDirParam createDirParam;
    createDirParam.bundleName = effectiveBundleName;  // dual-mode: use prefixed name for directories
    ...
    std::string dataBaseDir = ServiceConstants::BUNDLE_APP_DATA_BASE_DIR + ServiceConstants::BUNDLE_EL[1] +
        ServiceConstants::DATABASE + effectiveBundleName;
...
{
    // TODO: Switch to effective name after the Ark Native/AOT cache creation and update cleanup paths
    // are consistently adapted for dual-mode isolation.
    const std::string &bundleName = oldInfo.GetBundleName();
    std::string targetPath;
    targetPath.append(ServiceConstants::HAP_ARK_CACHE_PATH).append(bundleName);
    auto result = InstalldClient::GetInstance()->RemoveDir(
        targetPath, BundleDirScene::REMOVE_AOT_ARK_CACHE_DIR, bundleName);
    if (result != ERR_OK) {
...
ErrCode BaseBundleInstaller::RemoveBundleCodeDir(const InnerBundleInfo &info, const bool async) const
{
    ...
    (void)DelayedSingleton<InstallExceptionMgr>::GetInstance()->HandleBundleExceptionInfo(
        GetEffectiveBundleName(info));
    auto result = InstalldClient::GetInstance()->RemoveDir(
        info.GetAppCodePath(), BundleDirScene::REMOVE_BUNDLE_CODE_DIR, GetEffectiveBundleName(info), async);
    if (result != ERR_OK) {
    ...
    const InnerBundleInfo &info, bool forException, const bool async)
{
    sysEventInfo_.startTime = BundleUtil::GetCurrentTimeMs();
    ErrCode result =
        InstalldClient::GetInstance()->RemoveBundleDataDir(GetEffectiveBundleName(info), userId_,
    ...
    const bool isBundleUpdate) const
{
    for (const auto &item : infos) {
        ...
        // dual-mode: code dir is created under the prefixed name (CreateBundleCodeDir), so empty-dir
        // cleanup must target the same prefixed path (and the +new- rotate dir inherits it too).
        std::string effectiveBundleName = GetEffectiveBundleName(info);
        ...
        moduleDir += effectiveBundleName + ServiceConstants::PATH_SEPARATOR + info.GetCurrentModulePackage();
        ...
        if (isDirEmpty) {
            ...
            InstalldClient::GetInstance()->RemoveDir(
                moduleDir, BundleDirScene::REMOVE_MODULE_DIR, effectiveBundleName);
        }
    }
}
    ...
    const InnerBundleInfo &info, const std::string &modulePackage, int32_t userId, bool isKeepData) const
{
    ...
    // dual-mode: module/hap paths are already prefixed in info (CreateBundleCodeDir stores the prefixed
    ...
    // remove hap
    result = RemoveModuleDir(GetHapPath(info, info.GetModuleName(modulePackage)), effectiveBundleName);
    if (result != ERR_OK) {
...
void BaseBundleInstaller::InitDualModeBundleName(const InstallParam &installParam)
{
    ...
    // OTA/pre-install retry with forceDualModeCloneInstall: ResolveDualModePolicy is skipped for
    // OTA (isOTA gates its call), so set the role here to enable both clone naming
    // (ShouldUseDualModeCloneName) and cross-mode storage detection (IsCrossModeInstall).
    if (installParam.forceDualModeCloneInstall && dualModeInstallRole_ == DualModeInstallRole::NONE) {
...
void BaseBundleInstaller::InitDualModeBundleName(const InnerBundleInfo &bundleInfo)
{
    bundleName_ = bundleInfo.GetBundleName();
    dualModeBundleName_ = bundleInfo.IsDualModeCloneApp()
    ...
    std::unordered_map<std::string, InnerBundleInfo> &infos)
{
    ...
    // MAIN_ONLY / SUB_ONLY are mode-exclusive policies: the app belongs to the "other" mode when
    // the current mode doesn't match. Normal installs reject this (the app wouldn't be visible after
    // install). Pre-install must install both modes' apps regardless of the current boot mode —
    // cross-mode storage (IsCrossModeInstall + tempBundleInfos_) handles hiding the other-mode
    // variant until a mode switch makes it visible.
    ...
    // OTA/pre-install retry with forceDualModeCloneInstall: ResolveDualModePolicy is skipped for
    // OTA (isOTA gates its call), and InitDualModeBundleName runs after ParseHapFiles. Set the
    // role here so ShouldUseDualModeCloneName returns true and IsDualModeCloneApp is set on the
    // info — without this, AddInnerBundleInfo would look up installStates_ by the original name
    // instead of the clone key and fail ("bundle is not installed").
    if (installParam.forceDualModeCloneInstall && dualModeInstallRole_ == DualModeInstallRole::NONE) {
        dualModeInstallRole_ = DualModeInstallRole::SECONDARY;
    }
...
void BaseBundleInstaller::ResetInstallProperties()
{
    ...
    resolvedDeviceModeDistributionPolicy_ = DeviceModeDistributionPolicy::UNSPECIFIED;
    dualModeErmsCache_ = DualModeErmsCache{};
    tempInfo_ = InstallerBundleTempInfo{};
...
{
    const std::string asanLogDir = std::string(ServiceConstants::BUNDLE_ASAN_LOG_DIR) + ServiceConstants::PATH_SEPARATOR
        + std::to_string(userId_) + ServiceConstants::PATH_SEPARATOR + GetEffectiveBundleName(info);
    ErrCode errCode =
    ...
    uint32_t oldVersionCode, const std::string &oldNativeLibraryPath) const
{
    ...
    // dual-mode: old libs live under the prefixed code dir (CreateBundleCodeDir), so clean up the
    // prefixed path; GetEffectiveBundleName() equals bundleName_ in primary mode.
    const std::string &effectiveBundleName = GetEffectiveBundleName();
    std::string oldLibPath = std::string(Constants::BUNDLE_CODE_DIR) + ServiceConstants::PATH_SEPARATOR +
        effectiveBundleName + ServiceConstants::PATH_SEPARATOR + ServiceConstants::LIBS;
    if (InstalldClient::GetInstance()->RemoveDir(oldLibPath, BundleDirScene::REMOVE_BUNDLE_LIB_DIR,
        effectiveBundleName) != ERR_OK) {
...
std::string BaseBundleInstaller::GetTempHapPath(const InnerBundleInfo &info)
{
    ...
    if (isFeatureNeedUninstall_) {
        // dual-mode: +new- rotate dir is under the prefixed code dir, so use the effective name.
        // (hapPath itself is already prefixed via info.GetAppCodePath().)
        return std::string(Constants::BUNDLE_CODE_DIR) + ServiceConstants::PATH_SEPARATOR +
            std::string(ServiceConstants::BUNDLE_NEW_CODE_DIR) + GetEffectiveBundleName(info) +
    ...
    bool needCopyHapToInstallPath, bool needDeleteOldLibraryPath)
{
    ...
    // dual-mode: bundleName drives code-dir/so-dir paths and identifiers below; use the effective
    // (prefixed) name so libs/so dirs align with the prefixed code dir (CreateBundleCodeDir).
    std::string bundleName = GetEffectiveBundleName(infos.begin()->second);
    ErrCode result = ERR_OK;
    ...
    bool isBundleUpdate)
{
    ...
    // dual-mode: bundleName/bundleCodePathName drive code-dir/so-dir paths below; use the effective
    // (prefixed) name to align with the prefixed code dir (CreateBundleCodeDir).
    std::string bundleName = GetEffectiveBundleName();
    ...
    const bool isBundleUpdate) const
{
    ...
    // dual-mode: temp so path is under the prefixed code dir (CreateBundleCodeDir), so use the
    // effective (prefixed) name for both the path and the RemoveDir identifier.
    std::string effectiveBundleName = GetEffectiveBundleName(innerBundleInfo);
    std::string bundleCodePathName = isBundleUpdate ? (std::string(ServiceConstants::BUNDLE_NEW_CODE_DIR) +
        effectiveBundleName) : effectiveBundleName;
    std::string tempDir;
    ...
    if (isDirEmpty && (InstalldClient::GetInstance()->RemoveDir(
        tempDir, BundleDirScene::REMOVE_MODULE_DIR, effectiveBundleName)) != ERR_OK) {
    ...
    const InnerBundleInfo &info, const bool isBundleUpdate, const bool isModuleUpdate)
{
    if (isBundleUpdate) {
        // dual-mode: +new- rotate dir is under the prefixed code dir, so use the effective name.
        // (other branches use info.GetAppCodePath() which is already prefixed.)
        return std::string(Constants::BUNDLE_CODE_DIR) + ServiceConstants::PATH_SEPARATOR +
            std::string(ServiceConstants::BUNDLE_NEW_CODE_DIR) + GetEffectiveBundleName(info) +
    ...
    const std::string &modulePath, std::string &nativeLibraryPath, std::string &targetSoPath)
{
    ...
    // dual-mode: target so path is under the prefixed code dir (CreateBundleCodeDir), so use the
    // effective (prefixed) name for both the +new- rotate dir and the normal code dir.
    std::string effectiveBundleName = GetEffectiveBundleName(info);
    if (isBundleUpdate) {
        targetSoPath.append(Constants::BUNDLE_CODE_DIR).append(ServiceConstants::PATH_SEPARATOR)
            .append(ServiceConstants::BUNDLE_NEW_CODE_DIR).append(effectiveBundleName)
            .append(ServiceConstants::PATH_SEPARATOR).append(nativeLibraryPath)
            .append(ServiceConstants::PATH_SEPARATOR);
    } else {
        targetSoPath.append(Constants::BUNDLE_CODE_DIR).append(ServiceConstants::PATH_SEPARATOR)
            .append(effectiveBundleName).append(ServiceConstants::PATH_SEPARATOR).append(nativeLibraryPath)
            .append(ServiceConstants::PATH_SEPARATOR);
    }
}
...
{
    // dual-mode: ext profile dir is under the prefixed code dir (CreateBundleCodeDir), so use the
    // effective (prefixed) name; equals bundleName_ in primary mode.
    const std::string &effectiveBundleName = GetEffectiveBundleName();
    auto extProfileDir = std::string(Constants::BUNDLE_CODE_DIR) + ServiceConstants::PATH_SEPARATOR + effectiveBundleName
    ...
    if ((iter == installParam.parameters.end()) || (iter->second.empty())) {
        ...
        if (isExtProfileExist) {
            if (RemoveModuleDir(targetPath, effectiveBundleName) != ERR_OK) {
    ...
    ErrCode result =
        InstalldClient::GetInstance()->CreateBundleDir(effectiveBundleName, BundleDirScene::EXTEND_PROFILE_DIR, extProfileDir);
    if (result != ERR_OK) {
