...
ErrCode InstalldHostImpl::CreateSharefilesDataDirEl2(const CreateDirParam &createDirParam)
{
    ...
    if (SetDirApl(bundleShareFilesDataDir, bundleName, createDirParam.apl, hapFlags) != ERR_OK) {
        LOG_W(BMS_TAG_INSTALLD, "CreateSharefilesDataDirEl2 SetDirApl sharefiles failed: %{public}s",
            bundleName.c_str());
    }
...
ErrCode InstalldHostImpl::CreateBundleDataDir(const CreateDirParam &createDirParam)
{
    ...
    for (const auto &el : ServiceConstants::BUNDLE_EL) {
        ...
        if (CreateExtensionDir(createDirParam, bundleDataDir, S_IRWXU, createDirParam.gid) != ERR_OK) {
            LOG_W(BMS_TAG_INSTALLD, "create extension dir failed, parent dir %{public}s",
                bundleDataDir.c_str());
        }
