...
bool OnDemandInstallDataMgr::SaveOnDemandInstallBundleInfo(const PreInstallBundleInfo &preInstallBundleInfo)
{
    if (!onDemandDataStorage_->SaveOnDemandInstallBundleInfo(preInstallBundleInfo)) {
        APP_LOGE("save onDemandInstall bundle failed bundle:%{public}s", preInstallBundleInfo.GetBundleName().c_str());
        return false;
    }
    return true;
}
