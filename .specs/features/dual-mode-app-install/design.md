...
| 类别 7 | `APP_CATEGORY_DIFF_PACKAGE`（位值 32），同包名不同包体，双模式特殊处理对象 |
| dual-mode clone app | 副模式安装的类别 7 应用，`InnerBundleInfo.isDualModeCloneApp=true`，`appIndex=0` |
...
| 项 | 补充说明 |
|----|----------|
| 前缀机制定位 | 副模式类别7应用**走普通安装管线**（BaseBundleInstaller），非分身管线（BundleCloneInstaller）；仅在 bundleName 上拼接 `+clone-10000+` 前缀 |
...
| OH-ARCH-IPC-SAF | InstallParam 跨 IPC 传递 | appCategory 必须加入 Parcel 序列化（ReadFromParcel+Marshalling） | 单测 |
| OH-ARCH-API-LEVEL | 新增 Public API | ApplicationInfo.appCategory / InstallParam.appCategory / AppCategory，需 SysCap 声明 | API 评审/XTS |
| OH-ARCH-ERROR-LOG | 类别7互转/非系统应用安装失败需错误码 | 新增专用错误码 `ERR_APPEXECFWK_INSTALL_DUAL_MODE_CATEGORY_CONFLICT`(8519943) / `ERR_APPEXECFWK_INSTALL_DUAL_MODE_NOT_SYSTEM_APP`(8519942) | hilog/单测 |
...
| 维度 | 设计结论 |
|------|----------|
| 兼容性（涉及） | ApplicationInfo/InstallParam 新增字段带默认值（APP_CATEGORY_UNSPECIFIED=0）；`from_json` 找不到 key 回退默认值机制天然兼容存量（AC-18），零迁移 |
| API/SDK（涉及） | 3 个 Public 增量：见「API 签名」节；枚举按位或预留扩展位 |
| IPC/跨进程（涉及） | InstallParam.appCategory 经 Parcel 跨进程；目录/轮转操作经 InstalldClient（SA 511） |
...
**标识字段 — isDualModeCloneApp（ADR-9）**：`InnerBundleInfo` 新增 `bool isDualModeCloneApp_`（默认 false，Parcel/JSON 向后兼容）。安装时副模式类别7置 true；存储按字段定 DB key；加载用 DB 原始 key 保留主副两条；分类按字段+模式分入两 map。该字段是持久化标识，cross-flow 调用方（卸载/恢复的新 installer 实例）也能准确判定。
...
**类别7判定**：`(appCategory & 32) != 0`（`DualModeHelper::IsDiffPackageCategory`，详见「数据模型」枚举定义）。
...
### ADR-1：副模式类别7应用走普通安装管线 + bundleName 前缀（非分身管线）

| 字段 | 内容 |
|------|------|
| 问题 | 副模式类别7应用如何安装隔离？复用分身管线还是普通管线？ |
| 推荐方案 | 走普通安装管线（BaseBundleInstaller），在 bundleName_ 确定后、创建目录前，对副模式类别7应用的 bundleName 拼接 `+clone-10000+` 前缀 |
...
| 字段 | 内容 |
|------|------|
| 问题 | 副模式查询类别7应用时，是改查询 key 为带前缀，还是加载时去前缀？ |
| 推荐方案 | 加载时去前缀。副模式类别7应用加载后以**去前缀的原始 bundleName** 放入 `bundleInfos_`（当前模式可查询）；主模式下放入 `tempBundleInfos_`。查询接口不变，始终用原始 bundleName 查 |
...
| 边界补强 | `ClassifyDualModeAppsNoLock` 副模式分支：若某类别7应用**仅有 primary 变体（无 clone 对应）**，swap 逻辑会跳过它致其残留 bundleInfos_ 被错误查询；新增兜底遍历（bundle_data_mgr.cpp:400-411）将所有 `IsDiffPackageCategory && !IsDualModeCloneApp` 的条目从 bundleInfos_ 移入 tempBundleInfos_，确保副模式下 bundleInfos_ 仅保留 clone |
...
| 推荐方案 | 新建 `DualModeHelper`，封装：①读 ispcmode(0=tablet/1=2in1) + mainmode(0=主tablet/1=主2in1) 两个 int 参数；②判断主/副模式（`IsDualModeDevice=(ispcmode∈{0,1} && mainmode∈{0,1})`，`IsSecondaryMode=IsDualModeDevice && ispcmode≠mainmode`）；③判断应用类别是否需特殊处理；④前缀生成/解析 |
...
### ADR-8：AppCategory 枚举值设计（按位或 + 预留扩展）

| 字段 | 内容 |
|------|------|
| 问题 | 7 种应用类别枚举值如何定义以支持按位或？ |
| 推荐方案 | 类别 1（APP_CATEGORY_UNSPECIFIED）取值 0 表"不区分类别"，作默认值；类别 2~7 采用 2 的幂次（1/2/4/8/16/32），支持按位或组合 |
| 取舍理由 | UNSPECIFIED=0 天然表"无类别"；加载分类时显式判断 `appCategory == UNSPECIFIED` 使其在所有模式可查询，避免被位运算误隐藏；类别 2~7 幂次值预留扩展空间。放弃连续整数 1..7（不支持按位或） |
| 影响 | 需求文档示例用 1..7 是"类别编号"语义，实现枚举值为 0,1,2,4,8,16,32 |

### ADR-9：副模式应用标识字段 isDualModeCloneApp

| 字段 | 内容 |
|------|------|
| 问题 | 如何在 DB key 带前缀实现持久层隔离的同时，让加载分类能识别副模式类别7应用、把主副同名记录分别放入 bundleInfos_/tempBundleInfos_ 而不冲突？ |
| 推荐方案 | InnerBundleInfo 新增 `bool isDualModeCloneApp_`（默认 false，Parcel/JSON 向后兼容）。①安装：副模式类别7安装时置 true；②存储：按该字段决定 DB key（true→带前缀，false→原名）；③加载：TransResult 的 validInfos 用 DB 原始 key 做 key（保留主副两条不丢）；④分类：按 `IsDualModeCloneApp()` + 当前模式，去前缀原名分入 bundleInfos_/tempBundleInfos_ |
...
### ADR-12：类别7一致性校验收敛到双模式设备

| 字段 | 内容 |
|------|------|
| 问题 | 更新时的「类别7 ↔ 非类别7 互转拦截」（AC-8）在所有设备都生效，还是仅双模式设备？ |
| 推荐方案 | `BaseBundleInstaller::CheckDualModeCategoryConsistency`（base_bundle_installer.cpp:5766-5780）入口先判 `!DualModeHelper::IsDualModeDevice()` → 直接返回 ERR_OK。类别7互转拦截仅对双模式设备生效 |
...
### ADR-26：类别7 仅系统应用准入（SetDualModeAppInfo 校验 IsSystemApp）

| 字段 | 内容 |
|------|------|
| 问题 | 双模式类别7隔离是系统级能力。普通（非系统）应用若可配置类别7，会无意义占用隔离资源且语义错配。是否限制？ |
...
| 取舍理由 | 类别7隔离是系统级能力，限制系统应用对齐其语义；校验先于置位保证失败无副作用；非系统应用/主模式/非类别7/非双模式设备不触达，零回归 |
...
| 字段 | 内容 |
|------|------|
| 问题 | ADR-12 `CheckDualModeCategoryConsistency`（:5766-5780）仅校验当前模式侧（bundleInfos_）。副模式安装时另一模式变体在 tempBundleInfos_，类别不一致（cat7↔非cat7）会致主副模式类别冲突。如何补跨 map 维度？ |
| 推荐方案 | 新增 `CheckDualModeCategoryConsistencyInTemp`（base_bundle_installer.cpp:5782-5803），由 `InnerProcessBundleInstall`（:1063，紧随 :1059 当前模式侧）调用：`IsDualModeDevice` 守卫 → `FetchTempBundleInfo(bundleName_)` 查 tempBundleInfos_ 另一模式变体 → cat7↔非cat7 互转返回 `ERR_APPEXECFWK_INSTALL_DUAL_MODE_CATEGORY_CONFLICT`（8519943）；两模式均类别7 或另一模式不存在则放行 |
| 取舍理由 | 补 ADR-12 当前模式侧的跨 map 维度，与 AC-8（当前模式）配对成 AC-35（跨 map）；`IsDualModeDevice` 守卫使非双模式设备早退零回归；不误拦合法双模式（两模式均类别7 放行） |
...
flowchart TD
    ...
    F -- 是 --> H["Step1: 带前缀 cat7 → tempBundleInfos_<br/>key 改为去前缀原名"]
    H --> I{"IsSecondaryMode()?"}
    I -- 是 --> J["Step2 副模式:<br/>clone swap 回 bundleInfos_(原名key)<br/>兜底: 非clone cat7 primary → tempBundleInfos_ (:400-411)"]
...
|------|--------|----------|-----------|------|
| 1 | 安装方 | BundleMgrService | InstallParam.appCategory=类别7 | IPC |
...
| 6 | NotifyBundleStatus | BundleCommonEventMgr | 事件含 appCategory+currentMode(int)+共沙箱 | AC-17 |

### 数据模型

```typescript
// 应用类别枚举（类别2~7 按位或幂次位值；类别1 UNSPECIFIED=0 表默认无类别）
enum AppCategory {
  APP_CATEGORY_UNSPECIFIED = 0,            // 不区分（默认）
  APP_CATEGORY_TABLET_ONLY = 1,            // TABLET独有（需求"PAD独有"）
  APP_CATEGORY_2IN1_ONLY = 2,              // 2IN1独有（需求"PC独有"）
  APP_CATEGORY_TABLET_SUPPORT_2IN1 = 4,    // TABLET独有支持2IN1安装（需求"PAD支持PC"）
  APP_CATEGORY_2IN1_SUPPORT_TABLET = 8,    // 2IN1独有支持TABLET安装（需求"PC支持PAD"）
  APP_CATEGORY_SAME_PACKAGE = 16,          // 同包名同包体一多
  APP_CATEGORY_DIFF_PACKAGE = 32,          // 同包名不同包体一多（本需求特殊处理）
}
```
...
| 数据类型 | 存储方式 | 位置 | 生命周期 |
|----------|----------|------|----------|
| appCategory（应用类别） | InnerBundleInfo JSON → installed_bundle 表 VALUE | /bmsdb.db | 持久化 |
| isDualModeCloneApp（克隆标识） | InnerBundleInfo JSON → installed_bundle 表 VALUE | /bmsdb.db | 持久化 |
| 副模式类别7记录 key | installed_bundle 表 KEY（带前缀） | /bmsdb.db | 持久化 |
...
| 类别7互转更新 | BaseBundleInstaller | CHECK_RESULT | 返回 CATEGORY_CONFLICT 8519943（AC-8/AC-35） |
| 非系统应用装类别7 | SetDualModeAppInfo | CHECK_RESULT | 返回 NOT_SYSTEM_APP 8519942（AC-34） |
...
| 骨架项 | 目标 | 不包含 | 验证方式 |
|--------|------|--------|----------|
| API/接口骨架 | AppCategory 枚举、ApplicationInfo/InstallParam.appCategory 字段 + 序列化 | 完整模式判断业务逻辑 | 编译通过 + 序列化单测 |
...
| Task ID | 目标 | 受影响文件 | 依赖 |
|---------|------|------------|------|
| TASK-1 | 应用类别枚举 + ApplicationInfo/InstallParam 字段与序列化 | application_info.h/cpp, install_param.h/cpp | design+spec Approved |
...
|----------|------|-----|-----------|----------|--------|
| `enum AppCategory { APP_CATEGORY_UNSPECIFIED = 0, ... }` | Public（枚举） | BundleKit | `@ohos.bundle.bundleManager.d.ts` | - | BundleManager |
| `ApplicationInfo.appCategory: number` | Public（字段） | BundleKit | `@ohos.app.ability.ApplicationInfo.d.ts` | - | BundleManager |
| `InstallParam` 新增 `appCategory` 入参 | Public（字段） | BundleKit | `@ohos.bundle.installer.d.ts` | - | BundleManager |
...
| 错误码 | 码值 | 含义 |
|--------|------|------|
| `ERR_APPEXECFWK_INSTALL_DUAL_MODE_CATEGORY_CONFLICT` | 8519943 | 类别7 互转更新失败 / 跨 map 类别冲突（AC-8/AC-35） |
