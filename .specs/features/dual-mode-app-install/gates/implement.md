...
| 增量 AC-42~54（模式独占 8519947 / 预置 ERMS 两趟 fan-out / 跨模式变体存储 / 切换互斥 8519944 安装侧 / 查询回显 appIndex=10000 / NAPI 枚举注册） | ⏳ | 代码已落地（随分支演进合流至 `fix_dual_doc` HEAD `bcbfe06a9`；单测随代码入库：安装专项 install 164 + query 95 = 259 例），编译/单测/运行时回归待集成环境。AC-48~54 为 2026-09-04 规则检视拆分项（自 AC-41/43/45/17 拆出、行为零变化，代码/单测同上） |
| 无阻塞级 Open Issues | ✅ | review.md 历史 FAIL 均已修；剩余为 follow-up 遗留 |

## 总结论

**Approval（编译验证通过，`_04` 层）** — 代码落地完整（`_04` `80d089208` + 增量 `14eb7f286` + TASK-7 增量 + AC-42~54 增量（AC-48~54 为拆分项，行为零变化），当前基线 `fix_dual_doc` HEAD `bcbfe06a9`）、静态审查 PASS、`_04` 编译验证通过（112 例单测）；主体 AC-1~21 运行集成验证 PASS（2026-07-18）。未决：① 增量（AC-36~54 + AC-17 5 字段 + AC-19 appIndex 置位；单测随代码入库至安装专项 install 164 + query 95 = 259 例）编译/单测/运行时回归待集成环境；② 运行时全 AC（AC-1~54）集成回归。编译通过 ≠ 运行回归 PASS。
